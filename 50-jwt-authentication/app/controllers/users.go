...
func (c *User) registerUser(model *models.Models, useRole string, w http.ResponseWriter, r *http.Request) {
	...
	// validate password
	err = c.validator.Var(requestData.Password, "required,min=6")
	if err != nil {
		// write response error
		WriteResponse(w, http.StatusBadRequest, "User validation failed: password: required"+err.Error()+". Password must be at least 6 characters long.", true)
		return
	}
