...
func (c *Route) Create() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		err = c.model.UpsetRouteByFromAndTo("routes", &models.Route{
		...
		data, err := c.model.FindRouteByFromAndTo("routes", requestData.From, requestData.To)
		if err != nil {
			// write response error
...
func (c *Route) GetAll() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		routes, err := c.model.GetAllRoutes("routes", r.URL.Query().Get("from"),
			r.URL.Query().Get("to"))
		if err != nil {
			WriteResponse(w, 500, fmt.Sprintf("failed to get data from mongodb, %s", err.Error()), true)
			return
...
func (c *Route) Get() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		route, err := c.model.FindRouteById("routes", GetLastPathID(r))
		if err != nil {
			WriteResponse(w, 500, fmt.Sprintf("failed to get data from mongodb, %s", err.Error()), true)
			return
		}
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		for i, line := range data {
			if i > 0 { // omit header line
				...
				err = c.model.UpsetRouteByFromAndTo("routes", &rData)
				if err != nil {
					WriteResponse(w, http.StatusBadRequest, fmt.Sprintf("failed to insert or update data to mongodb, %s", err.Error()), true)
					return
				}
