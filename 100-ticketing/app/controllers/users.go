...
func (c *User) GetAll() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		user, err := c.model.GetAllUsers(r.URL.Query().Get("name"), GetPaginationOption(r))
		if err != nil {
...
func (c *User) Get() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		id := GetLastPathID(r)
		user, err := c.model.FindUserById(id)
		if err != nil {
			if err == mongo.ErrNoDocuments {
				WriteResponse(w, 400, fmt.Sprintf("user with id %s not found", id), true)
				return
			}
...
func (c *User) Put() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		user, err := c.model.FindUserById(GetLastPathID(r))
		if err != nil {
			WriteResponse(w, 500, fmt.Sprintf("failed to get data from mongodb, %s", err.Error()), true)
			return
		}
		...
		err = c.model.UpsetUserByEmail(&user)
		if err != nil {
...
func (c *User) Delete() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err := c.model.DeleteUserByID(user.Id); err != nil {
...
func (c *User) Me() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		user, err := c.model.FindUserById(userId)
		if err != nil {
			WriteResponse(w, 500, fmt.Sprintf("failed to get data from mongodb, %s", err.Error()), true)
			return
		}
...
func (c *User) Auth() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// find user by email
		user, err := c.model.FindUserByEmail(requestData.Email)
		if err != nil {
			if err == mongo.ErrNoDocuments {
				// write response error
...
func (c *User) registerUser(model *models.Models, useRole string, w http.ResponseWriter, r *http.Request) {
	...
	// find user by email
	_, err = model.FindUserByEmail(requestData.Email)
	if err != nil {
		if err != mongo.ErrNoDocuments {
			WriteResponse(w, 500, fmt.Sprintf("failed to get data from mongodb, %s", err.Error()), true)
			return
	...
	// find role
	role, err := model.FindRoleByName(useRole)
	if err != nil {
		WriteResponse(w, 500, fmt.Sprintf("failed to get data from mongodb, %s", err.Error()), true)
		return
	}

	err = model.UpsetUserByEmail(&models.UserList{
